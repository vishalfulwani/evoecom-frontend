...
const calculateAverageRating = (reviews: IReview[]): number => {
    if (reviews.length === 0) return 5; // No reviews, default to 0
    const totalRating = reviews.reduce((sum, review) => sum + review.rating, 0);
    return totalRating / reviews.length; // Average rating
};

const renderStars = (reviews: IReview[]) => {
    ...
    for (let i = 1; i <= totalStars; i++) {
        if (i <= Math.floor(rating)) {
            stars.push(<i key={i} className="bx bxs-star text-white"></i>); // Fully filled stars
        } else if (i === Math.ceil(rating) && rating % 1 !== 0) {
            stars.push(<i key={i} className="bx bxs-star-half text-white"></i>); // Half star
        } else {
            stars.push(<i key={i} className="bx bxs-star text-light-4"></i>); // Empty star
        }
    }
